...
def fetch_and_upload_exchange_rate(bucket_name: str, date: str) -> None:
    ...
    try:
        ...
        logger.info("Fetching exchange rate successfully!")

        data = exchange_rate_df.to_parquet()

        date = date or datetime.now().strftime("%Y-%m-%d")
        key = construct_minio_key(
            prefix_type="exchange_rate", date=date, file_format="parquet"
        )
        ...
        logger.exception("Error at fetch_exchange_rate module date=%s", date)
