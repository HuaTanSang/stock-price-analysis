...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"sync_market_data_{datetime.now().strftime('%Y%m%d')}.log"
        ),
    ],
)
...
def main():
    parser = argparse.ArgumentParser(
        description="Comprehensive Market Data Sync Orchestrator"
    )
    parser.add_argument(
        "--group",
        type=str,
        default="HOSE",
        help="Stock universe group (e.g., HOSE, HNX, UPCOM, VN30, VN100)",
    )
    parser.add_argument(
        "--mode",
        type=str,
        default="daily",
        choices=["init", "daily", "eod", "live"],
        help="Time window mode",
    )
    parser.add_argument(
        "--skip-reference", action="store_true", help="Skip reference data update"
    )
    parser.add_argument(
        "--skip-news", action="store_true", help="Skip news data update"
    )
    parser.add_argument(
    ...
    if not args.skip_news:
        tasks.append(
            [python_cmd, "-m", "vnstock_pipeline.cli", "run", "news", "--limit", "10"]
        )
    ...
    logger.info("=" * 60)
    logger.info("Starting Market Data Sync Pipeline")
