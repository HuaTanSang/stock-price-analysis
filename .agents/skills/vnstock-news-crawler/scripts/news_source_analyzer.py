...
def analyze(site, timeframe):
    if site not in SUPPORTED_SITES:
        print(
            f"Warning: {site} is not natively supported. Assuming it relies heavily on custom config."
        )
        return
    ...
    print(
        f"Capabilities: RSS={'Yes' if has_rss else 'No'}, Sitemap={'Yes' if has_sitemap else 'No'}"
    )
    ...
    if strategy == "rss":
        ...
        print("Use the following RSS URLs:")
        for s in sources:
            print(f" - {s}")
        ...
        print("articles = crawler.get_articles_from_feed(limit_per_feed=50)")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--site", type=str, required=True, help=f"Target site: {SUPPORTED_SITES}"
    )
    parser.add_argument(
        "--timeframe",
        type=str,
