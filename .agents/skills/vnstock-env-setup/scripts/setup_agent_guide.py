        ...
        if sys_os == "Darwin":
            ...
            run_cmd(
                [
                    "powershell",
                    "-Command",
                    "Invoke-WebRequest -Uri https://github.com/git-for-windows/git/releases/download/v2.44.0.windows.1/Git-2.44.0-64-bit.exe -OutFile git_installer.exe",
                ]
            )
            ...
            run_cmd(
                [
                    "powershell",
                    "-Command",
                    "Start-Process -FilePath .\\git_installer.exe -ArgumentList '/VERYSILENT /NORESTART /NOCANCEL /SP- /CLOSEAPPLICATIONS /RESTARTAPPLICATIONS' -Wait",
                ]
            )
...
def ensure_git_backup():
    if not os.path.exists(".git"):
        ...
        subprocess.run(
            ["git", "config", "user.email", "agent@vnstocks.com"], capture_output=True
        )
        subprocess.run(["git", "config", "user.name", "AI Agent"], capture_output=True)
        res = subprocess.run(
            ["git", "commit", "-m", "Initial commit before Agent Guide installation"],
            capture_output=True,
        )
        if res.returncode == 0:
        ...
        subprocess.run(["git", "config", "user.name", "AI Agent"], capture_output=True)
        res = subprocess.run(
            ["git", "commit", "-m", "Backup workspace before Agent Guide installation"],
            capture_output=True,
        )
        if res.returncode == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--confirm-docs-overwrite",
        action="store_true",
        help="Confirm overwriting the docs folder",
    )
    ...
    if os.path.exists("docs") and not args.confirm_docs_overwrite:
        print(
            "❌ ERROR: 'docs' directory exists. You must provide --confirm-docs-overwrite to proceed."
        )
        sys.exit(1)
