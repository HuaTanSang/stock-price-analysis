...
def step_1_update_market_data():
    ...
    try:
        if HAS_VNSTOCK_DATA and VNSTOCK_DATA_VERSION >= "3.0.0":
            ...
            symbols = (
                ref.equity.list()["symbol"].head(30).tolist()
            )  # Example: Get first 30 symbols
...
def step_2_fetch_news():
    ...
    try:
        ...
        filename = os.path.join(
            DATA_DIR, "news", f"news_{datetime.now().strftime('%Y%m%d')}.csv"
        )
...
def main():
    logger.info(
        f"=== Automation Started (vnstock_data: {VNSTOCK_DATA_VERSION if HAS_VNSTOCK_DATA else 'N/A'}) ==="
    )
