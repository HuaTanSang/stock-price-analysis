    ...
    try:
        ...
        if total_replacements > 0:
            if not dry_run:
                ...
                print(
                    f"🔍 [DRY RUN] Would migrate {filepath} ({total_replacements} replacements)"
                )
            return True
        return False
...
def get_python_files(directory: str):
    ...
    for root, _, filenames in os.walk(directory):
        ...
        if any(
            ignored in root
            for ignored in [".venv", "venv", "env", ".git", "__pycache__"]
        ):
            continue
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate Python files from vnstock to vnstock_data."
    )
    ...
    if modified_count == 0:
        ...
        if not args.dry_run:
            ...
            print(
                "> you may also want to manually update your code to use the Unified UI "
            )
