...
def generate_mock_data():
    ...
    # 3. Monte Carlo
    mc_matrix = (
        np.cumprod(1 + np.random.normal(0.0008, 0.015, (200, 252)), axis=1) * 100_000
    )
...
def run_demo():
    ...
    df, port_returns, equity, drawdown, benchmark, mc_matrix, alloc_df = (
        generate_mock_data()
    )
    ...
    c2 = BarChart(
        x=["Tháng 1", "Tháng 2", "Tháng 3", "Tháng 4", "Tháng 5"],
        y=[5.2, 10.1, -2.5, 8.4, 3.1],
        name="Lợi nhuận %",
        title=title2,
        theme="dark",
    )
    ...
    c4 = ScatterChart(
        x=np.random.uniform(5, 25, 50).tolist(),
        y=np.random.uniform(-10, 50, 50).tolist(),
        name="Stock",
        title=title4,
        theme="dark",
    )
    ...
    c7 = EquityCurve(title=title7, theme="dark", width=1200, height=500).build(
        equity_data=equity, drawdown_data=drawdown, benchmark=benchmark
    )
    ...
    c8 = ReturnsHistogram(
        returns=port_returns, bins=50, show_normal=True, title=title8, theme="dark"
    )
    ...
    c9 = MonthlyReturnsHeatmap(returns=port_returns, title=title9, theme="dark")
    ...
    c12 = AllocationChart(
        allocation_data=alloc_df, allocation_type="Ngành", title=title12, theme="light"
    )
    ...
    for i in range(1, 13):
        fname = [
            ...
            if f.startswith(f"{i:02d}_") and f.endswith(".html")
        ][0]
        filepath = os.path.join(output_dir, fname)
        with open(filepath, "r", encoding="utf-8") as f:
            content = f.read()
        content = content.replace(
            "<title>Awesome-pyecharts</title>", f"<title>Chart {i} Demo</title>"
        )
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(content)
