...
out_dir = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "docs", "assets", "gallery")
)
...
def save_chart(fig, name):
    ...
    fig.savefig(
        os.path.join(out_dir, f"{name}{suffix}.png"), bbox_inches="tight", dpi=150
    )
    plt.close(fig)
...
df_returns = pd.Series(
    np.random.normal(0.0006, 0.015, 1000),
    index=dates,
    name="Strategy" if args.lang == "en" else "Chiến lược",
)
df_bench = pd.Series(
    np.random.normal(0.0004, 0.012, 1000), index=dates, name="VN-Index"
)
...
title_7 = (
    "Portfolio Allocation (Top 10 Stocks)"
    if args.lang == "en"
    else "Phân bổ Tỷ trọng Danh mục (Top 10 Cổ phiếu)"
)
fig, ax = Chart.treemap(
    values=weights, labels=tickers, title=title_7, color_palette="flatui"
)
...
title_8 = (
    "Beta & Correlation: Strategy vs VN-Index"
    if args.lang == "en"
    else "Beta & Tương quan: Chiến lược vs VN-Index"
)
...
ax.plot(
    df_scatter["Benchmark"],
    m * df_scatter["Benchmark"] + b,
    color="#E57373",
    linewidth=2,
)
...
title_9 = (
    ...
    if args.lang == "en"
    else "Tăng trưởng Đa lớp Tài sản (Multi-Asset Class Growth)"
)
...
cols_10 = (
    ...
    if args.lang == "en"
    else ["Ngân hàng", "Chứng khoán", "Bất động sản"]
)
...
title_10 = (
    "Sector Returns Correlation"
    if args.lang == "en"
    else "Tương quan Lợi nhuận giữa các Nhóm ngành"
)
