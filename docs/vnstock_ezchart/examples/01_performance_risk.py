...
# 1. Equity Curve
title_1 = (
    "Portfolio vs VN-Index Performance (Equity Curve & Drawdown)"
    if args.lang == "en"
    else "Hiệu suất Danh mục vs VN-Index (Equity Curve & Drawdown)"
)
fig, ax1, ax2 = Chart.equity_curve(
    cum_returns, benchmark=cum_bench, title=title_1, figsize=(12, 7)
)
save_chart(fig, "01_equity_curve")

# 2. Returns Heatmap
title_2 = (
    "Seasonality Analysis: Monthly Returns Matrix"
    if args.lang == "en"
    else "Phân tích Tính Chu Kỳ: Ma trận Lợi nhuận Hàng tháng"
)
fig, ax = Chart.returns_heatmap(df_returns, title=title_2, figsize=(10, 5))
...
title_4 = (
    "Risk Management: Rolling Volatility (30-day)"
    if args.lang == "en"
    else "Quản trị Rủi ro: Độ biến động (Rolling Volatility 30-ngày)"
)
...
title_5 = (
    ...
    if args.lang == "en"
    else "Phân phối Lợi nhuận hàng ngày (Daily Returns Distribution)"
)
