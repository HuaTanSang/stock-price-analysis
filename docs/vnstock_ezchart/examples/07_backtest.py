...
portfolio = pd.DataFrame({"equity": equity, "drawdown": drawdown}, index=dates)
...
out_dir = os.path.join(
    os.path.dirname(os.path.dirname(__file__)), "docs", "assets", "gallery"
)
...
def generate_backtest_chart(lang):
    ...
    title = (
        "VN-Index: Backtest Strategy Results"
        if lang == "en"
        else "VN-Index: Kết quả Backtest Chiến lược"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        choices=["vi", "en"],
        help="Language for chart labels",
    )
