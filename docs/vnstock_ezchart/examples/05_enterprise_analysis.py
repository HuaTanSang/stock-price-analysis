...
revenue = pd.Series(
    [1200, 1350, 1100, 1500, 1400, 1600], index=quarters, name=rev_label
)
...
title_9 = (
    ...
    if args.lang == "en"
    else "Phân tích Cơ bản: Doanh thu & Biên Lợi nhuận Gộp"
)
...
title_17 = (
    ...
    if args.lang == "en"
    else "So sánh Hiệu quả Sinh lời (ROE & ROA)"
)
...
top_stocks = pd.DataFrame(
    {
        ...
        "Signal" if args.lang == "en" else "Tín hiệu": signals_11_en
        if args.lang == "en"
        else signals_11_vi,
    }
)
...
title_11 = (
    "Top Stock Signal Rankings"
    if args.lang == "en"
    else "Xếp hạng Tín hiệu Cổ phiếu Tích cực"
)

fig = Chart.table(
    top_stocks, title=title_11, colWidths=[0.2, 0.2, 0.2, 0.3], figsize=(7, 4)
)
...
cols_18 = (
    ...
    if args.lang == "en"
    else ["HĐ Kinh doanh", "HĐ Đầu tư", "HĐ Tài chính"]
)
df_cf = pd.DataFrame({cols_18[0]: cfo, cols_18[1]: cfi, cols_18[2]: cff})

title_18 = (
    "Corporate Cash Flow Structure Analysis"
    if args.lang == "en"
    else "Phân tích Cấu trúc Dòng tiền Doanh nghiệp"
)
