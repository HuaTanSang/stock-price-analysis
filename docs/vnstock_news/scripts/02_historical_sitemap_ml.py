...
async def fetch_historical_news_for_ml():
    ...
    print(
        ">> Use Case: Thu thập kho dữ liệu khổng lồ (vài tháng đến nhiều năm) chuyên phục vụ phân tích dữ liệu AI (NLP / Machine Learning).\n"
    )
    ...
    sitemap_url = config.get("sitemap_url") or config.get("sitemap", {}).get(
        "current_url"
    )
