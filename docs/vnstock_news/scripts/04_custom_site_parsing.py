...
async def custom_site_crawl():
    ...
    print(
        ">> Use Case: Cần linh hoạt cào ngay lập tức 1 trang chứng khoán/tin tức chuyên biệt mà không cần chờ tác giả update bản phân phối.\n"
    )
    ...
    crawler = AsyncBatchCrawler(
        custom_config=custom_cfg, max_concurrency=2, debug=False
    )
